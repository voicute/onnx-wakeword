...
> 中文行为误触发优化版模型；非中文行为基础版演示模型（误触发优化开发测试中）。
...
**误触发优化：优化前后对比**（同一测试语料：挖掘语料 1/10 留出集，15.4 小时语音/音乐/混音；裸模型、阈值 0.5、40ms 滑窗、按触发文件计）：

| 关键词 | 优化前 (次/小时) | 优化后 (次/小时) | 降幅 | 召回变化 |
|---------|---:|---:|---:|---|
...
本仓库内置的对比模型（`models/zh/`）：

| 关键词 | 优化前基线 (`*_r0.onnx`) | 误触发优化版 (`*_r1.onnx`) |
|---------|---------|---------|
...
> 自行验证：分别加载同一个关键词的基线与优化版，播放音乐或视频对白——基线频繁误触发，优化版保持安静。**基线仅作对比，正式使用请选优化版。**

> 误触发优化目前已在**中文**关键词上完成生产验证；**英语 / 日语 / 法语 / 德语：开发测试中**。
